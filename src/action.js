...
function markdownReport(reports, commit, options) {
  ...
  ].map((row) => `| ${row.filter(Boolean).join(" | ")} |`).join("\n");
  output += "\n";
...
async function pullRequestInfo(payload = {}) {
  ...
  if (pullRequestNumber) {
    ...
    // Find the PR from the ref
    const { data } = await client.rest.pulls.list({
      ...github.context.repo,
      state: "open",
    });
    const branchName = payload.ref.replace("refs/heads/", "");
    const pullRequest = data.find(pr => pr.head.ref === branchName);
    if (pullRequest) {
      pullRequestNumber = pullRequest.number;
      commit = pullRequest.head.sha;
    }
